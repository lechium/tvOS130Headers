/*
* This header is generated by classdump-dyld 1.0
* on Tuesday, November 5, 2019 at 2:39:58 AM Mountain Standard Time
* Operating System: Version 13.0 (Build 17J586)
* Image Source: /System/Library/PrivateFrameworks/CoreAccessories.framework/Support/accessoryd
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <accessoryd/ACCFeatureServer.h>
#import <accessoryd/NSXPCListenerDelegate.h>

@class _ACCNavigationProviderInfo, NSMutableDictionary, NSString;

@interface ACCNavigationServer : ACCFeatureServer <NSXPCListenerDelegate> {

	_ACCNavigationProviderInfo* _navigationProviderInfo;
	NSMutableDictionary* _navigationProviderInfoList;
	NSMutableDictionary* _navigationProviderInfoListByProviderUID;
	NSMutableDictionary* _registeredAccessoryConnections;

}

@property (nonatomic,retain) _ACCNavigationProviderInfo * navigationProviderInfo;                          //@synthesize navigationProviderInfo=_navigationProviderInfo - In the implementation block
@property (nonatomic,readonly) NSMutableDictionary * navigationProviderInfoList;                           //@synthesize navigationProviderInfoList=_navigationProviderInfoList - In the implementation block
@property (nonatomic,readonly) NSMutableDictionary * navigationProviderInfoListByProviderUID;              //@synthesize navigationProviderInfoListByProviderUID=_navigationProviderInfoListByProviderUID - In the implementation block
@property (nonatomic,readonly) NSMutableDictionary * registeredAccessoryConnections;                       //@synthesize registeredAccessoryConnections=_registeredAccessoryConnections - In the implementation block
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
+(id)sharedServer;
-(void)dealloc;
-(BOOL)listener:(id)arg1 shouldAcceptNewConnection:(id)arg2 ;
-(BOOL)shouldAcceptXPCConnection:(id)arg1 ;
-(void)sendUpdatedSubscriberList;
-(id)initWithXPCServiceName:(id)arg1 andFeatureNotification:(const char*)arg2 ;
-(void)accessoryNavigationAttached:(id)arg1 componentList:(id)arg2 ;
-(void)accessoryNavigationDetached:(id)arg1 ;
-(void)accessoryNavigationStartRouteGuidance:(id)arg1 componentIdList:(id)arg2 options:(unsigned long long)arg3 ;
-(void)accessoryNavigationStopRouteGuidance:(id)arg1 componentIdList:(id)arg2 ;
-(void)accessoryNavigationObjectDetectionUpdateInfo:(id)arg1 componentIdList:(id)arg2 updateInfo:(id)arg3 ;
-(void)iterateAttachedConnectionsSync:(/*^block*/id)arg1 ;
-(NSMutableDictionary *)registeredAccessoryConnections;
-(void)notifyOfProvider:(id)arg1 connection:(id)arg2 ;
-(_ACCNavigationProviderInfo *)navigationProviderInfo;
-(NSMutableDictionary *)navigationProviderInfoList;
-(void)setNavigationProviderInfo:(_ACCNavigationProviderInfo *)arg1 ;
-(void)iterateNavigationProviderListSync:(/*^block*/id)arg1 ;
-(NSMutableDictionary *)navigationProviderInfoListByProviderUID;
@end

